...
load_uci() {
    ...
    uci_get() {
        ...
        local v
        v=$(uci -q get "cfiptools.${section}.${key}" 2>/dev/null)
        printf '%s' "${v:-$default}"
    }
    ...
    CFG_github_push_retries=$(uci_get github_push_retries "3")
    CFG_github_retry_delay_sec=$(uci_get github_retry_delay_sec "10")
...
run_test() {
    ...
    if [ "${CFG_download_input:-1}" = "1" ] && [ -n "${CFG_input_url:-}" ]; then
        ...
        curl -sL --connect-timeout "$DOWNLOAD_TO" --max-time "$((DOWNLOAD_TO * 2))" \
            ...
            -o "$INPUT_FILE" \
            "${CFG_input_url}" >> "$LOG_FILE" 2>&1 || log "Download failed, using existing file"
    fi
    ...
    ARGS="$ARGS --output \"$FULL_OUTPUT\""
    ARGS="$ARGS --best-output \"$BEST_OUTPUT\""
    ARGS="$ARGS --tcp-timeout $TCP_TIMEOUT_SEC"
    ARGS="$ARGS --tcp-workers ${CFG_tcp_workers:-200}"
    ARGS="$ARGS --speed-timeout ${CFG_speed_timeout_sec:-6.0}"
    ARGS="$ARGS --speed-workers ${CFG_speed_workers:-5}"
    ARGS="$ARGS --min-speed ${CFG_min_speed_mbps:-16}"
    ARGS="$ARGS --top ${CFG_top_per_region:-10}"
    ARGS="$ARGS --show-latency ${CFG_show_latency:-true}"
    ARGS="$ARGS --show-mbps ${CFG_show_bandwidth:-false}"

    if [ "${CFG_numbered_regions:-0}" = "1" ]; then
        ARGS="$ARGS --NO"
    fi
    if [ "${CFG_verbose:-0}" = "1" ]; then
        ARGS="$ARGS --verbose"
    fi
    ...
    # 【新增】：测速一结束，立即撤销防拦截规则，让后续的 GitHub 上传恢复走代理！
    cleanup_proxy_bypass
    ...
    if [ "${CFG_update_readme:-0}" = "1" ]; then
        ...
        python3 "$DATA_DIR/update_md.py" >> "$LOG_FILE" 2>&1 || log "README generation failed (ignored)"
    fi
