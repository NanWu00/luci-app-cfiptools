...
function index()
    if not nixio.fs.access("/etc/config/cfiptools") then
        return
    end

    entry({"admin", "services", "cfiptools"},
        alias("admin", "services", "cfiptools", "main"),
        _("CFIP优选"), 90)

    entry({"admin", "services", "cfiptools", "main"},
        template("cfiptools/main"),
        _("CFIP优选"), 10)

    entry({"admin", "services", "cfiptools", "save"},
        call("action_save"))

    entry({"admin", "services", "cfiptools", "start_test"},
        call("action_start_test"))

    entry({"admin", "services", "cfiptools", "stop_test"},
        call("action_stop_test"))

    entry({"admin", "services", "cfiptools", "get_status"},
        call("action_get_status"))

    entry({"admin", "services", "cfiptools", "log_view"},
        call("action_log_view"))

    entry({"admin", "services", "cfiptools", "log_poll"},
        call("action_log_poll"))

    entry({"admin", "services", "cfiptools", "clear_log"},
        call("action_clear_log"))

    -- [已删除冗余的 log_stream 路由，杜绝僵尸进程]

    entry({"admin", "services", "cfiptools", "reset"},
        call("action_reset"))

    entry({"admin", "services", "cfiptools", "read_file"},
        call("action_read_file"))

    entry({"admin", "services", "cfiptools", "manual_upload"},
        call("action_manual_upload"))
end
...
function action_save()
    ...
    local checkbox_fields = {
        "enabled", "download_input", "github_upload_enabled",
        "show_latency", "show_bandwidth", "numbered_regions",
        "verbose", "update_readme"
    }
    ...
    -- 注意：去掉了 cron_interval，直接接收前端算好的 cron_schedule
    local fields = {
        "cron_schedule",
        "input_url", "download_timeout", "input_file",
        "full_output_file", "best_output_file",
        "tcp_timeout_ms", "tcp_workers",
        "speed_timeout_sec", "speed_workers", "min_speed_mbps",
        "top_per_region", "max_nodes",
        "fast_label",
        "readme_file", "raw_base_url",
        "test_location", "update_frequency",
        "github_repo", "github_branch",
        "github_token", "github_message",
        "git_http_proxy", "git_https_proxy",
        "bypass_proxy_method", "pre_test_command", "post_test_command"
    }
    ...
    for _, name in ipairs(fields) do
        ...
        if val ~= nil then
            if val == "" then
                cursor:delete("cfiptools", "config", name)
            else
                cursor:set("cfiptools", "config", name, val)
            end
        end
    end
    ...
    -- 极限优化：检测如果是前端 AJAX 请求，仅返回极简 JSON 即可，省去渲染整个网页的巨大开销
...
function action_get_status()
    ...
    if nixio.fs.access(status_file) then
        status = luci.sys.exec("cat " .. status_file):match("^(%S+)") or "空闲"
    end
...
function action_log_poll()
    ...
    if nixio.fs.access(log_file) then
        ...
        local lines = {}
        for line in raw:gmatch("[^\r\n]+") do
            table.insert(lines, 1, luci.util.pcdata(line))
        end
...
function action_reset()
    ...
    local defaults = {
        ...
        download_input = "1",
        download_timeout = "30",
        input_file = "/usr/share/cfiptools/ips.txt",
        full_output_file = "/usr/share/cfiptools/full_ips.txt",
        best_output_file = "/usr/share/cfiptools/best_ips.txt",
        update_readme = "1",
        readme_file = "/usr/share/cfiptools/README.MD",
        raw_base_url = "",
        test_location = "",
        update_frequency = "",
        tcp_timeout_ms = "500",
        ...
        numbered_regions = "1",
        verbose = "0",
        github_upload_enabled = "0",
        github_repo = "",
        github_branch = "main",
        github_token = "",
        github_message = "Update IP and README",
        git_http_proxy = "",
        git_https_proxy = "",
        bypass_proxy_method = "nftables",
        pre_test_command = "",
        post_test_command = "",
        last_run = "",
        last_result = ""
    ...
    for k, v in pairs(defaults) do
        cursor:set("cfiptools", "config", k, v)
    end
...
function action_read_file()
    ...
    if path == "" then
        luci.http.write_json({ error = "路径为空" })
        return
    end

    if path:find("%.%.") then
        luci.http.write_json({ error = "非法路径" })
        return
    end
    ...
    if not allowed then
        ...
        if (best and path == best) or (full and path == full) or (readme and path == readme) then
            allowed = true
        end
    end

    if not allowed then
        luci.http.write_json({ error = "路径不被允许" })
        return
    end
...
function action_manual_upload()
    ...
    local cmd = string.format([[
        (
            ...
            echo "[$(date +'%%Y-%%m-%%d %%H:%%M:%%S')] [手动触发] 开始通过 Python API 直连上传 GitHub..." >> /var/log/cfiptools.log
