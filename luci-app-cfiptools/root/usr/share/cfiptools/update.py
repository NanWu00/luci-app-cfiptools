            ...
            try:
                ...
                if latency is not None:
                    ...
                    if verbose and IS_TTY:
                        ...
                        print(f"[LAT] {node.raw} -> {latency} ms")
                progress.update(1)
            finally:
                queue.task_done()
    ...
    async def worker() -> None:
        while True:
            ...
            try:
                ...
                # 核心改动：直接 await 原生异步测速函数，不用开启消耗资源的 thread
                ...
                if verbose:
                    ...
                    if IS_TTY:
                        ...
                        print(f"[SPEED] {candidate.node.raw} -> {speed} Mbps {status}")
                progress.update(1)
            finally:
                queue.task_done()
    ...
    *,
    ...
    with path.open("w", encoding="utf-8", newline="\n") as file:
        ...
        for result in results:
            # 1. 自动匹配国旗 Emoji，如果字典里没有，默认使用 🌐 地球图标
            emoji = REGION_EMOJIS.get(result.node.region.upper(), "🌏")

            # 2. 计算国家代码及编号 (例如 HK_1, HK_2)
            ...
            # 3. 构建测速数据的后缀标签 (例如 [⚡ 100ms | 50M])
            label = build_label(result, show_latency=show_latency, show_mbps=show_mbps, fast_label=fast_label)
            suffix = f" {label}" if label else ""

            # =========================================================
            # 4. 最终输出格式组合 (请根据你的喜好保留其中一行，删掉或注释掉另外两行)
            # =========================================================
            
            # 【方案 A：最美观推荐】软件列表里显示为： 🇭🇰 HK_1 [⚡ 100ms]
            file.write(f"{result.node.ip}:{result.node.port}#{emoji} {region_name}{suffix}\n")
            
            # 【方案 B：带上 IP】如果你希望在软件列表名称里直接看到 IP，显示为： 🇭🇰 HK_1 1.2.3.4 [⚡ 100ms]
            # file.write(f"{result.node.ip}:{result.node.port}#{emoji} {region_name} {result.node.ip}{suffix}\n")
            
            # 【方案 C：绝对靠前】强制放到 IP 前面。警告：这会破坏绝大多数代理软件的导入标准！
            # file.write(f"{emoji} {region_name} {result.node.ip}:{result.node.port}#{suffix}\n")
            # 【默认方案】
